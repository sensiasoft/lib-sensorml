 ...
 * assuming object-to-image mappin
 ...
 */
/*
 * 2006/05/11, MEB, corrected, cleaned up, and validated
 * 2007/10/05, MEB, updated for revised schema
 * 2008/01/03, MEB, updated for revised schema
...
public class RPC_Process extends ExecutableProcessImpl
{
	...
	public void init() throws SMLProcessException
	{
		try
		{
			...
			DataRecord rpcParamSet = (DataRecord) rpcParams.getComponent(0);

			// read image region
			DataRecord rpcImageRegion = (DataRecord) rpcParamSet.getComponent("image_region");
			...
			DataRecord rpcImageAdj = (DataRecord) rpcParamSet.getComponent("image_adjustment");
			...
			DataRecord rpcTargetAdj = (DataRecord) rpcParamSet.getComponent("target_adjustment");
			...
			// read all coefs
			DataRecord rpcCoefsXNum = (DataRecord) rpcParamSet.getComponent("x_numerator_coefficients");
			x_num_coefs = new RPCCoef[rpcCoefsXNum.getComponentCount()];
			readCoefs(rpcCoefsXNum, x_num_coefs);

			DataRecord rpcCoefsXDen = (DataRecord) rpcParamSet.getComponent("x_denominator_coefficients");
			x_den_coefs = new RPCCoef[rpcCoefsXDen.getComponentCount()];
			readCoefs(rpcCoefsXDen, x_den_coefs);

			DataRecord rpcCoefsYNum = (DataRecord) rpcParamSet.getComponent("y_numerator_coefficients");
			y_num_coefs = new RPCCoef[rpcCoefsYNum.getComponentCount()];
			readCoefs(rpcCoefsYNum, y_num_coefs);

			DataRecord rpcCoefsYDen = (DataRecord) rpcParamSet.getComponent("y_denominator_coefficients");
			y_den_coefs = new RPCCoef[rpcCoefsYDen.getComponentCount()];
			readCoefs(rpcCoefsYDen, y_den_coefs);

			// read errors
			DataRecord rpcErrorParams = (DataRecord) rpcParamSet.getComponent("error_parameters");
	...
	protected void readCoefs(DataRecord coefGroup, RPCCoef[] coefArray)
	{
